...
function watchLogs(v){
  setTimeout(() => {
    if(v != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团25-12 17点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
  ...
  for(let i=0;i<h5guardData.length;i++){
    ...
    setTimeout(()=>{
      axios.get(h5guardData[i]["uurl"]).then((response) => {
        let time = new Date(toDateString(new Date).slice(0,11) + "16:59:59").getTime() - new Date().getTime();
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  ...
                  ee.setEnvVar("mt_sq25_12_17",headers["Cookie"]);
...
autoCheck.autoCheck("mt_sq25_12_17",'美团25-12 17点通知').then((res)=>{
  ck = process.env["mt_sq25_12_17"] || "";
  if(ck != "")
    getCookies();
});
