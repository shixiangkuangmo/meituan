...
let headers = {};
const sq_coupon = {
  couponId: "C6C509946857448CBD610EE4866903E5",
  gdPageId: "474800",
  pageId: "475929",
  instanceId: "16807805191810.06901521494526408",
  componentId: "16807805191810.06901521494526408",
  desc:"天天神券-社群专享 25-12 14:00:00"
};
...
function watchLogs(v){
  setTimeout(() => {
    if(v != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团25-12 15点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
  ...
  for(let i=0;i<h5guardData.length;i++){
    ...
    setTimeout(()=>{
      axios.get(h5guardData[i]["uurl"]).then((response) => {
        let time = new Date(toDateString(new Date).slice(0,11) + "14:59:59").getTime() - new Date().getTime();
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  ...
                  ee.setEnvVar("mt_sq25_12_15",headers["Cookie"]);
...
autoCheck.autoCheck("mt_sq25_12_15",'美团25-12 15点通知').then((res)=>{
  ck = process.env["mt_sq25_12_15"] || "";
  if(ck != "")
    getCookies();
});
