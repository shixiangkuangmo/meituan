...
function watchLogs(v){
  setTimeout(() => {
    if(v != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团38-16 17点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  ...
                  ee.setEnvVar("mt_sq38_16_17",headers["Cookie"]);
...
autoCheck.autoCheck("mt_sq38_16_17",'美团38-16 17点通知').then((res)=>{
  ck = process.env["mt_sq38_16_17"] || "";
  if(ck != "")
    getCookies();
});
