...
function watchLogs(v){
  setTimeout(() => {
    if(v != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团30-15 16点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  ...
                  ee.setEnvVar("mt_sq30_15_16",headers["Cookie"]);
...
autoCheck.autoCheck("mt_sq30_15_16",'美团30-15 16点通知').then((res)=>{
  console.log(res);
  ck = process.env["mt_sq30_15_16"] || "";
  if(ck != "")
    getCookies();
});
