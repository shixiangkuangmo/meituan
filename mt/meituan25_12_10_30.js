...
function watchLogs(v){
  setTimeout(() => {
    ...
    if(len != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团25-12 10:30点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
  ...
  for(let i=0;i<h5guardData.length;i++){
    ...
    setTimeout(()=>{
      axios.get(h5guardData[i]["uurl"]).then((response) => {
        let time = new Date(toDateString(new Date).slice(0,11) + "10:29:59").getTime() - new Date().getTime();
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  logTimes[i] = qqTimes;
                  ee.setEnvVar("mt_sq25_12_10_30",headers["Cookie"]);
...
autoCheck.autoCheck("mt_sq25_12_10_30","美团25-12 10:30点通知").then((res)=>{
  ck = process.env["mt_sq25_12_10_30"] || "";
  if(ck != "")
    getCookies();
});
