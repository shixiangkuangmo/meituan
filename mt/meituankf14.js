...
let headers = {};
const sq_coupon = {
  couponId: "BB96372C3EE345A19F3ADD02F491BAB3",
  gdPageId: "528552",
  pageId: "532755",
  instanceId: "16956330325720.05233830716458732",
  componentId: "16956330325720.05233830716458732",
  desc:"开工第一杯---通用红包 -- 满25.0减25.0---（14:00:00）"
};
...
function watchLogs(v){
  setTimeout(() => {
    ...
    if(len != cookies.length*qqTimes){
      ...
      sendNotify.sendNotify("美团咖啡14点通知:",msgstr);
    }
  }, 100);
}
let intervals = [];
  ...
  for(let i=0;i<h5guardData.length;i++){
    ...
    setTimeout(()=>{
      axios.get(h5guardData[i]["uurl"]).then((response) => {
        let time = new Date(toDateString(new Date).slice(0,11) + "13:59:59").getTime() - new Date().getTime();
            ...
            axios
              ...
              .then((answer) => {
                ...
                if(answer.data.msg.indexOf("成功")>-1){
                  logTimes[i] = qqTimes;
                  ee.setEnvVar("mt_kf14",headers["Cookie"]);
...
autoCheck.autoCheck("mt_kf14",'美团咖啡14点通知').then((res)=>{
  ck = process.env["mt_kf14"] || "";
  if(ck != "")
    getCookies();
});
